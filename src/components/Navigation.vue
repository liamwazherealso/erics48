    ...
    <nav class="container py-5 px-4 flex flex-col gap-4 justify-between items-center sm:flex-row">
      ...
      <div class="flex items-center gap-x-4">
        ...
        >
          ...
          <img
            src="../assets/images/icons/hiker.png"
            alt="Hiker Image"
            class="w-14 hover:border-white"
          />
        </router-link>
      ...
      <ul
        :class="[
          'flex flex-col gap-4 sm:flex sm:flex-row sm:gap-x-10',
          { hidden: !isMenuOpen },
          'sm:flex-row sm:gap-x-10'
        ]"
        class="sm:flex sm:flex-row sm:gap-x-10"
      >
        <!-- Router link to LogHike page if user is logged in -->
        <router-link
          v-if="user"
          class="hover:text-beige cursor-pointer"
          :to="{ name: 'LogHike' }"
        >
          Log Hike
        </router-link>
